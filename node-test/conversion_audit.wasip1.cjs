...
const __wasmCandidates = [
  __nodePath.join(__dirname, "conversion_audit.wasm32-wasip1.debug.wasm"),
  __nodePath.join(__dirname, "conversion_audit.wasm32-wasip1.wasm"),
  __nodePath.join(__dirname, "zig-out", "node", "conversion_audit.wasm32-wasip1.debug.wasm"),
  __nodePath.join(__dirname, "zig-out", "node", "conversion_audit.wasm32-wasip1.wasm"),
];
...
function __resolvePackagedWasmPath() {
  const __wasmName = "conversion_audit.wasm32-wasip1.wasm";
  try {
    const __entry = require.resolve("zig-napi-node-test-wasm32-wasip1");
  ...
  try {
    return require.resolve("zig-napi-node-test-wasm32-wasip1/conversion_audit.wasm32-wasip1.wasm");
  } catch {}
  return undefined;
}
...
if (!__wasmFilePath) {
  throw new Error(
    "Cannot find conversion_audit.wasm32-wasip1.wasm next to this loader, in zig-out/node, or in the zig-napi-node-test-wasm32-wasip1 package.",
  );
}
...
function __annotateWasiInstantiationError(error) {
  if (error && typeof error.message === "string" && /memory|linkerror/i.test(error.message)) {
    error.message +=
      "\n[zig-napi] " +
      "the loader allocates WebAssembly.Memory(initial: 4000 pages, maximum: 65536 pages); the linked wasm must declare the same limits (zig-napi passes -Dwasi-initial-memory-pages / -Dwasi-max-memory-pages from napi.wasm.initialMemory / maximumMemory)";
  }
  return error;
}
