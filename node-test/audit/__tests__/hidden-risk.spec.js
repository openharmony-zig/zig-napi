...
const asyncAddon = require(loadPath)("async_audit");
const isWasi =
...
function isolated(body, flags = []) {
  return spawnSync(
    ...
    const asyncAddon=require(${JSON.stringify(loadPath)})('async_audit');
...
nativeTest("later argument failure releases the already-created TSFN", (t) => {
  const result = isolated(`
    assert.throws(()=>asyncAddon.queueThreadSafeFunction(()=>{},'bad',1));
    console.log('caught');
  `);
...
test("omitted event listener behaves like explicit undefined and null", async (t) => {
  t.is(await asyncAddon.asyncSliceEvents(1), 1);
  t.is(await asyncAddon.asyncSliceEvents(1, undefined), 1);
  t.is(await asyncAddon.asyncSliceEvents(1, null), 1);
});
...
test("event listener rejection preserves arbitrary JavaScript thrown values", async (t) => {
  for (const reason of [
    ...
    const outcome = await asyncAddon
...
test("AbortSignal removal exceptions do not leave the task pending", async (t) => {
  ...
  const outcome = await Promise.race([
    asyncAddon.asyncAbortable(0, signal).then(
...
test("unobserved progress does not allocate a payload for every event", async (t) => {
  await asyncAddon.asyncSliceEvents(1, undefined);
  const before = asyncAddon.allocationCount();
