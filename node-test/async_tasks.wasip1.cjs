...
const __wasmCandidates = [
  __nodePath.join(__dirname, "async_tasks.wasm32-wasip1.debug.wasm"),
  __nodePath.join(__dirname, "async_tasks.wasm32-wasip1.wasm"),
  __nodePath.join(__dirname, "zig-out", "node", "async_tasks.wasm32-wasip1.debug.wasm"),
  __nodePath.join(__dirname, "zig-out", "node", "async_tasks.wasm32-wasip1.wasm"),
];
...
function __resolvePackagedWasmPath() {
  const __wasmName = "async_tasks.wasm32-wasip1.wasm";
  try {
  ...
  try {
    return require.resolve("zig-napi-node-test-wasm32-wasip1/async_tasks.wasm32-wasip1.wasm");
  } catch {}
  return undefined;
}
...
if (!__wasmFilePath) {
  throw new Error(
    "Cannot find async_tasks.wasm32-wasip1.wasm next to this loader, in zig-out/node, or in the zig-napi-node-test-wasm32-wasip1 package.",
  );
}
