...
if (!__wasmFilePath) {
  try {
    __wasmFilePath =
      require.resolve("zig-napi-node-test-wasm32-wasi/classes_audit.wasm32-wasi.wasm");
  } catch {
    throw new Error(
      "Cannot find classes_audit.wasm32-wasi.wasm file, and zig-napi-node-test-wasm32-wasi package is not installed.",
    );
  }
}
