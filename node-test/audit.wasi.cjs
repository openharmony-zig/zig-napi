...
const __wasmCandidates = [
  __nodePath.join(__dirname, "audit.wasm32-wasi.debug.wasm"),
  __nodePath.join(__dirname, "audit.wasm32-wasi.wasm"),
  __nodePath.join(__dirname, "zig-out", "node", "audit.wasm32-wasi.debug.wasm"),
  __nodePath.join(__dirname, "zig-out", "node", "audit.wasm32-wasi.wasm"),
];
...
function __resolvePackagedWasmPath() {
  const __wasmName = "audit.wasm32-wasi.wasm";
  try {
  ...
  try {
    return require.resolve("zig-napi-node-test-wasm32-wasi/audit.wasm32-wasi.wasm");
  } catch {}
  return undefined;
}
...
if (!__wasmFilePath) {
  throw new Error(
    "Cannot find audit.wasm32-wasi.wasm next to this loader, in zig-out/node, or in the zig-napi-node-test-wasm32-wasi package.",
  );
}
