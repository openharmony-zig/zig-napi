...
if (!__wasmFilePath) {
  try {
    __wasmFilePath = require.resolve("zig-napi-node-test-wasm32-wasi/async_audit.wasm32-wasi.wasm");
  } catch {
    throw new Error(
      "Cannot find async_audit.wasm32-wasi.wasm file, and zig-napi-node-test-wasm32-wasi package is not installed.",
    );
  }
}
