// Child harness for `async-lifecycle.spec.js`.
//
...
//
// usage: node async-child.cjs <scenario> <wasm file> <threaded|threadless>
const fs = require("fs");
...
function instantiate() {
  ...
  if (threaded) {
    ...
    options.onCreateWorker = () => {
      const worker = new Worker(path.join(__dirname, "async-worker.mjs"), {
        env: process.env,
      });
