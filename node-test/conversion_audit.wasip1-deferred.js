 ...
 * (e.g. `import mod from "./conversion_audit.wasm32-wasip1.wasm"` under a CompiledWasm module
  ...
  try {
    ...
    throw new TypeError(
      "instantiate() and createInstance() expect a precompiled WebAssembly.Module (or a Promise resolving to one), " +
        "e.g. import mod from './conversion_audit.wasm32-wasip1.wasm' under a CompiledWasm module rule / wrangler module import. " +
  ...
  if (__contextInitializationFailed) {
    ...
    try {
      ...
      __attachCleanupError(
        __registrationFailed ? __registrationError : __contextInitializationError,
        error,
      );
      try {
        __registerManagedBeforeExitListener();
...
async function __createInstance(__wasmInput, __beforeExitDestroy, __onManagedDestroyer) {
  ...
  try {
    ...
    let __napiModule;
    ({ instance: __napiInstance, napiModule: __napiModule } = await __emnapiInstantiateNapiModule(
      __emnapiModule,
      {
        context: __emnapiContext,
        asyncWorkPoolSize: 0,
