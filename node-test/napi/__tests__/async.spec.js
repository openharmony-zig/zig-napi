// Spec for the async/abort/TSFN/runtime contracts: async ownership, AbortSignal
// handling, task cancellation, TSFN shutdown, promise settlement and shared
...
const native = require(loaderPath)("async_tasks");
const workerSource = path.join(__dirname, "async-worker.js");
...
test("events without a listener are neither cloned nor queued", async (t) => {
  ...
  // events, so the producer must not pay for them.
...
test("an omitted listener with many events allocates almost nothing", async (t) => {
  ...
  // and its completion (the resource-lifecycle fixture allows 128 calls for half
  // that many events).
  const allocationDelta = native.allocationCount() - before;
      ...
      for(let i=0;i<10;i++){
        const worker=new Worker(${JSON.stringify(
          `const {parentPort}=require('worker_threads');
           const a=require(${JSON.stringify(loaderPath)})("async_tasks");
...
abortTest(
  ...
  async (t) => {
    ...
    // the environment that started them.
    await delay(1500);
    t.is(await native.asyncThreadValue(1), 2);
  },
);
