//! Fixture for the class wrapper and the binary wrappers.
//!
//! Contracts covered here:
//!   per environment class constructor references
//!   static methods, factories and ClassWithoutInit construction
//!   constructor rollback and setter replacement ownership
//!   detached / invalidated backing stores for TypedArray, DataView,
//!   ArrayBuffer and Buffer.
//!
//! The addon is built with a counting allocator so that the leak assertions in
//! `node-test/napi/__tests__/classes.spec.js` measure real allocations.
...
// ---------------------------------------------------------------------------
// Default field construction, setter replacement and rollback
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// init, static methods, value receivers, factories and static values
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Binary wrappers must re-validate after JavaScript reentry
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Worker data: captured by default, borrowed explicitly
// ---------------------------------------------------------------------------
...
/// the same block to each other, which the pattern check reports - the
...
// ---------------------------------------------------------------------------
// The runner's owned result is released on every path
// ---------------------------------------------------------------------------
...
comptime {
    napi.NODE_API_MODULE("classes", @This());
}
