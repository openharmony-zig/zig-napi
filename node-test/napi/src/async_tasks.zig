//! Exports for the async/abort/TSFN/runtime contracts: ownership of captured
//! inputs and results, promise settlement, cancellation, abort signals and
//! thread-safe-function queue cleanup.
//!
...
/// pending: a destroyed operation must still settle it.
...
/// captured state: a descriptor must not be reused after consumption.
...
// ---------------------------------------------------------------------------
// Event queue, timeout and shutdown probes
// ---------------------------------------------------------------------------
...
/// Largest number of events one operation keeps in flight.
pub fn eventQueueLimit() u32 {
    return @intCast(SliceTask.async_max_inflight_events);
}

/// Highest number of in-flight events observed since the last reset.
...
/// is cancelled; used to keep an environment busy while it is torn down.
...
/// task and its controller are still running.
...
const QueuedTsfn = napi.ThreadSafeFunction(TsfnArgs, TsfnReturn, true, 0);
...
pub fn queueThreadSafeFunction(tsfn: *QueuedTsfn, value: u32, count: u32) !void {
    var index: u32 = 0;
...
pub fn queueThreadSafeFunctionAbandon(tsfn: *QueuedTsfn, count: u32) !void {
    var index: u32 = 0;
    while (index < count) : (index += 1) {
...
// ---------------------------------------------------------------------------
// Shared runtime environment ownership
// ---------------------------------------------------------------------------
...
comptime {
    napi.NODE_API_MODULE("async_tasks", @This());
}
