//! Conversion layer fixture.
//!
//! Every export here exercises one contract of the conversion and ownership
//! layer: status checks, strict numeric ranges, partial conversion rollback,
//! borrowed/owned returns and the fixed array / void callback holes.
...
/// A plain (non union) argument must be validated too: the type gate applies to
/// ordinary parameters as well.
...
comptime {
    napi.NODE_API_MODULE("conversion", @This());
}
