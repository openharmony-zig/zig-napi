...
export function exerciseSyncWrappers(native: NativeAddon) {
  for (let i = 0; i < 250; i++) {
    ...
    assertEqual(
      native.call_function((left: number, right: number) => left + right),
      42,
      "function callback",
    );
    assertEqual(
    ...
    assertEqual(
      native.function_reference_ref_count((left: number, right: number) => left + right),
      2,
      "reference ref count",
    );
