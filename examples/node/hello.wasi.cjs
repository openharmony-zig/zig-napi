...
function __resolvePackagedWasmPath() {
  const __wasmName = "hello.wasm32-wasi.wasm";
  try {
    const __entry = require.resolve("zig-napi-node-example-wasm32-wasi");
  ...
  try {
    return require.resolve("zig-napi-node-example-wasm32-wasi/hello.wasm32-wasi.wasm");
  } catch {}
  return undefined;
}
...
if (!__wasmFilePath) {
  throw new Error(
    "Cannot find hello.wasm32-wasi.wasm next to this loader, in zig-out/node, or in the zig-napi-node-example-wasm32-wasi package.",
  );
}
