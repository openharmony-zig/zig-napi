...
const __wasmCandidates = [
  __nodePath.join(__dirname, "hello.wasm32-wasip1.debug.wasm"),
  __nodePath.join(__dirname, "hello.wasm32-wasip1.wasm"),
  __nodePath.join(__dirname, "zig-out", "node", "hello.wasm32-wasip1.debug.wasm"),
  __nodePath.join(__dirname, "zig-out", "node", "hello.wasm32-wasip1.wasm"),
];
...
function __resolvePackagedWasmPath() {
  const __wasmName = "hello.wasm32-wasip1.wasm";
  try {
    const __entry = require.resolve("zig-napi-node-example-wasm32-wasip1");
  ...
  try {
    return require.resolve("zig-napi-node-example-wasm32-wasip1/hello.wasm32-wasip1.wasm");
  } catch {}
  return undefined;
}
...
if (!__wasmFilePath) {
  throw new Error(
    "Cannot find hello.wasm32-wasip1.wasm next to this loader, in zig-out/node, or in the zig-napi-node-example-wasm32-wasip1 package.",
  );
}
