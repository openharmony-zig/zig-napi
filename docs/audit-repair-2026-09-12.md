...
> 历史快照：后续审计在 `7343594` 上继续发现并修复了生命周期、资源回滚及内存问题。请以[第二轮隐藏风险验收](hidden-risk-repair-2026-09-12.md)的最终范围、测试数量和限制为准；本页的旧验收结果不构成对后续发现的否定。
...
公开转换入口（`Napi.from_napi_value*`、`NapiValue.As`）自身也是事务：存在未提交的事务帧时并入该帧（自动参数转换的多个参数共享同一事务），否则新建局部事务并在成功后提交。因此在原生函数体内手工转换失败时，之前已创建的引用会被回滚；成功时引用归调用方所有。回归用例：函数体内 `value.As(ManualHolder)` 部分失败后 100/100 对象可回收；内层导出函数成功持有的引用在外层调用失败后仍然存活（`storeReference` + getter 重入）。
...
- 转换事务覆盖 `Function.New` 生成的导出函数回调，以及所有公开转换入口（`Napi.from_napi_value*`/`NapiValue.As`）：在没有未提交事务帧时它们会自建局部事务，成功后提交、失败时回滚。仍未覆盖的是**没有**安装事务帧的多参数入口：类构造器/方法逐个转换参数，第二个参数失败时第一个参数已提交的引用不会被回滚，需要类层安装 `Napi.ConversionFrame`（见“补充修复”一节的 helper 说明）。
...
- 事务回滚在原生清理之前执行，而带自定义 `deinit` 的转换值会在清理阶段释放自己拥有的 native 内存：自定义 `deinit` 不得释放本次转换创建的引用/TSFN（那些由事务释放、提交后归函数体所有），否则会与事务重复释放。已用 `NativeHolder` fixture 覆盖“自定义 deinit 只释放自己的 native 缓冲区 + 事务释放引用”的组合。
- TSFN 的 null-env 排空路径只使用队列项自带的 allocator 与错误快照：Node 22 先调用用户 finalizer（释放 wrapper）再以 null env 排空队列，因此 `cb` 在检查 `inner_env == null` 之前不得解引用 `context`。当前实现保持该顺序。
