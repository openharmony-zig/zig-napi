...
| F04 | 明确普通返回值借用、`Owned(T)` 返回值转移所有权；参数由调用边界释放；异步输入独立克隆，输出及事件保留原 allocator | 同步/异步字面量、别名、堆返回值、未捕获参数；实际文件读取示例和 GC 后计数回基线 |
...
| F10 | TSFN 的 null-env 排空路径只回收 native 数据；投递失败和正常交付使用明确的载荷所有权 | TSFN 正常投递、关闭/终止子进程及 native 清理单测 |
...
| F16 | 默认构建实际编译 host addons；提供 check/test/test-unit；接入 4 组审计 fixture、原生单测和独立包 CI | 82 项既有 JS 测试扩展至 173 项；根目录测试命令真正执行编译和测试 |
...
| 验收项 | 命令/方式 | 结果 |
| --- | --- | --- |
| Debug，Node 22.23.2 | `zig build test --summary all` | 173 项 JS + 24 项 Zig 单测通过 |
| ReleaseSafe，Node 22.23.2 | `zig build test -Doptimize=ReleaseSafe --summary all` | 173 项 JS + 24 项 Zig 单测通过 |
| ReleaseFast，Node 22.23.2 | `zig build test -Doptimize=ReleaseFast --summary all` | 173 项 JS + 24 项 Zig 单测通过 |
...
| WASM/emnapi，Node 22.23.2 | CLI 构建 `wasm32-wasip1-threads`，随后 `NAPI_RS_FORCE_WASI=error pnpm --filter zig-napi-node-test run test:run` | 165 项通过，8 项 native-only 测试显式跳过 |
...
更详细的示例与契约已同步至网站的 Conversion、Ownership、Async Runtime、Binary Data、Primitive Values 和 Objects 文档；历史转换重构计划已标记为历史设计。
