...
pub fn backingIsDetached(env: napi.napi_env, raw: napi.napi_value) !bool {
    if (comptime options.selectedNapiVersion().isAtLeast(.v7)) {
        ...
        if (status == napi.napi_ok) {
            ...
            if (comptime options.isNodeAddon() and !options.isWasmNodeAddon()) {
                // Node 12/14/16 used Data()==nullptr, which also classifies a
                // valid zero-length backing store as detached. Ask the runtime
                // to construct a JS view: valid empty buffers succeed; detached
                // buffers throw. Only clear the exception generated by this
                // compatibility probe; lookup failures remain real exceptions.
                var version: [*c]const napi.napi_node_version = null;
                const version_status = napi.napi_get_node_version(env, &version);
                if (version_status != napi.napi_ok) return NapiError.failStatus(version_status);
                if (version != null and version[0].major < 18) {
                    var global: napi.napi_value = null;
                    var constructor: napi.napi_value = null;
                    var view: napi.napi_value = null;
                    var s = napi.napi_get_global(env, &global);
                    if (s != napi.napi_ok) return NapiError.failStatus(s);
                    s = napi.napi_get_named_property(env, global, "Uint8Array", &constructor);
                    if (s != napi.napi_ok) return NapiError.failStatus(s);
                    ...
                    if (s != napi.napi_ok) return NapiError.failStatus(s);
                    if (!typed) return BinaryError.InvalidBinaryValue;
                    return false;
                }
            }
            return true;
