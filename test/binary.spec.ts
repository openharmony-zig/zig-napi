...
export function testBinary(native: NativeAddon) {
  ...
  assertEqual(native.get_arraybuffer_as_string(arrayBufferValue).length, 1024, "arraybuffer string length");
  ...
  assertEqual(native.sum_float32_typedarray(new Float32Array([1.5, 2.5, -1])), 3, "float32 typedarray sum");
