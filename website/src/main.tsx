...
function resolveRoute(): SiteRoute {
  return window.location.pathname === '/api' || window.location.pathname.startsWith('/api/') ? 'api' : 'home'
}
  ...
  return (
    <header className="site-header sticky top-0 z-50 border-b border-(--color-border)">
      <div className="container-page flex h-14 items-center justify-between gap-4">
        <a className="flex shrink-0 items-center gap-2.5 text-sm font-semibold text-(--color-fg)" href="/">
          <img className="size-7 rounded-md" src="/logo/icon-256.png" alt="" />
          zig-napi
        </a>
        <div className="ml-auto flex items-center justify-end gap-5">
          <nav className="flex items-center gap-5 text-sm text-(--color-muted)" aria-label="Primary navigation">
    ...
    <section className="relative min-h-[calc(100vh-3.5rem)] overflow-hidden">
      ...
      <div className="container-page grid min-h-[calc(100vh-3.5rem)] items-center gap-12 py-16 lg:grid-cols-[0.9fr_1.1fr]">
        <div className="reveal is-visible">
          <img className="mb-7 size-16 rounded-2xl border border-(--color-border-strong)" src="/logo/icon-512.png" alt="" />
          ...
          <p className="mt-6 max-w-2xl text-lg leading-8 text-(--color-muted)">
            Build N-API modules with Zig, ship OpenHarmony artifacts, compile Node.js addons, and generate TypeScript
            declarations from the same exported surface.
          </p>
      ...
      <div className="container-page grid gap-10 lg:grid-cols-[0.85fr_0.55fr] lg:items-end">
        <div>
          ...
          <p className="mt-5 max-w-2xl leading-7 text-(--color-muted)">
            The package exports both the <code>napi</code> module and the <code>napi_build</code> helpers used by examples
            in this repository.
          </p>
        </div>
...
function BuildSection({ activeSnippet, activeSnippetId, copyLabel, onCopy, onSnippetChange }: BuildSectionProps) {
...
function TypesSection() {
  return (
    <section id="types" className="section-band">
      <div className="container-page">
        <SectionHeader eyebrow="Type generation" title="TypeScript declarations stay close to Zig exports" />
        <div className="mt-9 grid gap-8 md:grid-cols-2">
          <div className="border-l-2 border-(--color-accent) pl-5">
            ...
            <p className="mt-3 leading-7 text-(--color-muted)">
              Functions, classes, tuples, structs, enums, arrays, unions, async descriptors, and raw N-API wrappers are
              mapped into declaration files during the build.
            </p>
          </div>
          <div className="border-l-2 border-(--color-accent) pl-5">
            <h3 className="text-lg font-semibold text-(--color-fg)">Override when the public contract differs</h3>
            <p className="mt-3 leading-7 text-(--color-muted)">
              <code>napi.dts(value, "TypeScriptType")</code> and <code>napi.Dts(T, "TypeScriptType")</code> let exported
              declarations express a custom TypeScript-facing contract while keeping runtime values unchanged.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="mt-8 overflow-hidden rounded-xl border border-(--color-border-strong)">
          {surfaces.map(([name, detail]) => (
            <div
              ...
              role="row"
            >
              <div className="bg-(--color-surface-1) px-5 py-4 font-semibold text-(--color-fg)" role="cell">
                {name}
              </div>
    ...
    <section className="section-band">
      <div className="container-page grid gap-8 lg:grid-cols-[0.75fr_0.5fr] lg:items-center">
        <div>
          ...
          <p className="mt-5 max-w-2xl leading-7 text-(--color-muted)">
            The standalone API reference covers module registration, build helpers, runtime values, binary wrappers,
            async descriptors, native ownership, errors, and custom TypeScript declaration overrides.
          </p>
        </div>
        <div className="panel">
...
function SectionHeader({ eyebrow, title }: { eyebrow: string; title: string }) {
  return (
    <div className="max-w-3xl">
      ...
      <h2 className="mt-4 text-(--text-h2) font-semibold leading-[1.1] text-(--color-fg)">{title}</h2>
    </div>
  )
}
